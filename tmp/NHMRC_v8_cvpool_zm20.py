...
def save_zoom_up_large_images():
    ...
    for x in range(minXY[0], maxXY[0], N_XY):
        for y in range(minXY[1], maxXY[1], N_XY):
    ...
    pool = mp.Pool()
    for i in range(NUM_CPUS):
        pool.apply_async(merge_image_list_tmp, args=(i, filename_list_nested[i], fn_dict, FROM_DIR, TO_DIR, IMGSIZE, N_XY))
...
def merge_image_list_tmp(cpuid, filename_list, fn_dict, FROM_DIR, TO_DIR, IMGSIZE, N_XY=10):
    print(f'cpuid: {cpuid}')
    print(f'len(filename_list): {len(filename_list)}')
    cols = rows = N_XY
